...
> بیلد بگیرد، دیباگ کند و چیزی را نشکند. آخرین بروزرسانی: **2026-08-26 — نسخه 3.6.3**.
...
# 3) تست‌ها — ۷۴ تست، همه آفلاین، ~۲۰ ثانیه
.\gradlew.bat test
...
└─ desktop/                          ← کل اپ
   ...
   ├─ src/test/kotlin/vpn/core/      ← ۷۴ تست (بخش ۷ توضیح می‌دهد کدام چه چیزی را می‌گیرد)
   └─ src/main/
...
1. **پسورد p12**: `CLIENT_P12_PASS="ikev2"` در setup-ikev2.sh == `SshService.CLIENT_P12_PASSWORD`.
...
| **تست‌های واحد** | ✅ **۷۴ تست، همه آفلاین، ~۲۰ ثانیه** (`.\gradlew.bat test`) |
...
2. **رمز `.p12` ثابت `"ikev2"`** (`Ssh.kt:37` == `CLIENT_P12_PASS` در setup-ikev2.sh) —
   در مخزن عمومی است، پس محافظتی نیست. کار لازم: تولید per-install تصادفی و عبور دادنش
   از طریق argv اسکریپت + ذخیره در فیلد `p12Pass` که همین حالا DPAPI دارد.
3. **`kill()` و تشخیص زنده‌بودن با نام image و پورت باز کار می‌کنند**، نه PID دقیق.
   نتیجه: `taskkill /IM xray.exe` هر xray دیگری که کاربر اجرا کرده را می‌کشد، و هر
   پروسه‌ای که پورت ۱۰۸۰۸ را گرفته باشد «متصل» خوانده می‌شود (و `verifyTraffic` از
   همان listener غریبه رد می‌شود). کار لازم: هدف‌گیری PID با اعتبارسنجی زنده‌بودن.
...
1. این فایل را کامل بخوان (مخصوصاً بخش ۵ — درس‌های دیباگ؛ بندهای ۲۴/۲۵/۲۶ تازه‌ترین‌اند).
...
4. `.\gradlew.bat test` — باید ۷۴ تست سبز باشد. بعد `createDistributable`.
