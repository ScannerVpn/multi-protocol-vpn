...
> بیلد بگیرد، دیباگ کند و چیزی را نشکند. آخرین بروزرسانی: **2026-09-03 — دسکتاپ 3.6.16 · اندروید 0.2.0 (فاز ۲)**.
...
از ۳ سپتامبر ۲۰۲۶ اندروید **تونل واقعی دارد** (فاز ۲) و روی ایمولاتور با ترافیک واقعی
تأیید شده است. سند مرجعش `android/README.md` و خلاصه‌ی معماری/درس‌ها در `PLAN.md` §۹.۱.

```powershell
cd android
.\fetch-core.ps1                        # یک بار: AAR هسته (~۱۰۷MB، در git نیست)
.\gradlew.bat :app:testDebugUnitTest    # ۲۶ تست JVM
.\gradlew.bat :app:assembleDebug
```

هسته همان `hiddify-core` است که ویندوز استفاده می‌کند (داخلش sing-box 1.13) — یعنی
...
نکته‌ی دیباگی که به دسکتاپ هم مربوط است: **یک هسته‌ی Go داخل JNI شکستش را با استک‌تریس
