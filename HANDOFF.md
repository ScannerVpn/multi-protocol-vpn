...
> بیلد بگیرد، دیباگ کند و چیزی را نشکند. آخرین بروزرسانی: **2026-09-03 — دسکتاپ 3.6.16 · اندروید 0.3.0 (فاز ۳، برابری قابلیت)**.
...
# 2) تست‌ها — ۲۷۸ تست، همه آفلاین، ~۵۰ ثانیه
.\gradlew.bat test
...
### اپ اندروید (پوشه `android/`)

از ۳ سپتامبر ۲۰۲۶ اندروید **تونل واقعی دارد و از نظر قابلیت با ویندوز برابر است**
(نسخه 0.3.0)، و روی ایمولاتور زنده تأیید شده. سند مرجعش `android/README.md` و
خلاصهٔ معماری/درس‌ها در `PLAN.md` §۹.۱ و §۹.۲.

```powershell
cd android
.\fetch-core.ps1                        # یک بار: AAR هسته (~۱۰۷MB، در git نیست)
.\gradlew.bat :app:testDebugUnitTest    # ۷۴ تست JVM
.\gradlew.bat :app:assembleDebug
```

هسته همان `hiddify-core` است که ویندوز استفاده میکند (داخلش sing-box 1.13) — یعنی
**اسکیمای کانفیگ دو سمت یکی نیست**: دسکتاپ فعلاً روی شکل قدیمی‌تر می‌نویسد و اندروید روی
۱.۱۲+. اگر روزی هسته‌ی دسکتاپ هم ارتقا یافت، `BoxConfigSchemaTest` اندروید نشان می‌دهد
چه چیزهایی عوض شده‌اند (پنج مورد؛ فهرست در `android/README.md`).

**سه جا که اندروید از ویندوز جلوتر است** (و اگر روزی دسکتاپ را بهتر کردید، الگو
همین است): سوییچ کانفیگ بدون قطع اتصال (`selectOutbound` روی هستهٔ زنده)، پینگ
واقعی همهٔ کانفیگ‌ها با یک `urlTest` روی گروه selector، و اسپلیت per-app که خود
اندروید اعمال میکند (نه تطبیق نام پروسه).

**دو واگرایی عمدی:** ۱) WireGuard/AmneziaWG روی اندروید از `endpoints[]` سینگ‌باکس
میگذرد، نه wireproxy — چون هستهٔ ۱.۱۳ خودش amneziawg-go را دارد و اندروید پروسهٔ
دومی برای اجرا ندارد. ۲) حالت‌های proxy-only/system-proxy و پورت محلی روی اندروید
وجود ندارند (VpnService خودش TUN است).

نکتهٔ دیباگی که به دسکتاپ هم مربوط است: **یک هسته‌ی Go داخل JNI شکستش را با استک‌تریس
جاوا اعلام نمی‌کند.** بدون `Libbox.redirectStderr` + فیلتر `E Go` در logcat، یک پنیک
(مثلاً `OverrideOptions` نال یا zone در آدرس IPv6) صرفاً به‌شکل «پروسه مرد» یا «تایم‌اوت
...
└─ desktop/                          ← کل اپ
   ...
   ├─ src/test/kotlin/vpn/core/      ← ۲۷۸ تست (بخش ۷ توضیح می‌دهد کدام چه چیزی را می‌گیرد)
   └─ src/main/
...
| **تست‌های واحد** | ✅ **۲۷۸ تست، همه آفلاین، ~۵۰ ثانیه** (`.\gradlew.bat test`) — تنها `AppListReproTest` روی غیرویندوز خودش را skip می‌کند (kernel32) |
...
4. `.\gradlew.bat test` — باید ۲۷۸ تست سبز باشد. بعد `createDistributable`.
