...
object ProxyPorts {
    ...
    // ANDROID PORT (diverges from desktop): the desktop reads the persisted
    // base port via Storage(settings.json); the Android app has no settings
    // store yet, so the validated default is used. Re-sync when settings land.
    @Volatile
    private var current: Int = sanitize(10808)
