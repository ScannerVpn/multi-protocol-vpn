...
>
> آخرین به‌روزرسانی: **۲ سپتامبر ۲۰۲۶ — دور ۹ (نسخه 3.6.17):** همگام‌سازی اسناد، پاس گرم برای
> تثبیت «Fastest»، لغو+پیشرفت Ping-all، نجات subscriptions.json، صیقل دیالوگ بستن — همه کامیت شدند.
> ⚠️ `createDistributable` نسخهٔ 3.6.17 هنوز اجرا نشده چون اپ در حال اجراست (فایل EXE قفل است) —
> با اپ بسته: `gradlew createDistributable`. (دور ۸ = 3.6.16: دیالوگ X + ریشه‌ای باگ پینگ + رنگ پینگ.)
...
- **۲۷۸ تست، ۰ شکست** — `gradlew test` آفلاین ~۵۰ ثانیه (نتایج: `desktop/build/test-results/test`).
...
5. **صیقل دیالوگ بستن (بدهی §۸-۹):** حذف chevron، انتقال Cancel به ستون محتوا، ریتم ۱۰/۱۴dp،
   کنتراست hint. رندر-فقط؛ `CloseBehavior` و ۱۳ تستش دست‌نخورده.
⚠️ باقی ماندهٔ دور ۹: `createDistributable` (اپ باید بسته باشد) + اعتبارسنجی زندهٔ Spearman پاس گرم
با `LIVE_PING_TEST=1` + انتخاب بنر تلگرامی جدید (۴ پیش‌نویس در `desktop/banners/drafts/`).
...
- **فاز بعدی:** SSH provision (فاز ۴)، همگرایی KMP (فاز ۵).

## ۹.۲ نسخه اندروید (فاز ۳ — ۳ سپتامبر ۲۰۲۶، نسخه 0.3.0 — برابری قابلیت)

وضعیت: **هر قابلیت کاربردیِ نسخه ویندوز روی اندروید هست، و سه مورد بهتر است.**
۷۴ تست JVM سبز (۳۰ اسکیما + ۱۲ WgConf + ۸ Backup + ۶ Store + ۶ ConfigSort +
۶ پارسر + ۶ Pinger). همه روی ایمولاتور زنده تأیید شد.

- **پینگ واقعی هر کانفیگ** (فاز ۳ روادمپ بسته شد): همهٔ کانفیگ‌ها داخل یک گروه
  `selector` رندر میشوند و `CommandClient.urlTest` از داخل هر کدام یک درخواست
  واقعی به `cp.cloudflare.com/generate_204` میزند. در حالت قطع، یک کانفیگ
  **بدون TUN** بالا میآید تا اندازه‌گیری ترافیک کاربر را دست نزند. اندازه‌گیری
  زنده: کانفیگ سالم `433 ms`، دو کانفیگ سیاه‌چاله «تایم‌اوت».
- **سوییچ کانفیگ بدون قطع اتصال:** `selectOutbound` روی هستهٔ زنده (دسکتاپ برای
  این کار مجبور است هسته را بکشد و از نو وصل شود). `interrupt_exist_connections`
  روشن است وگرنه سوکت‌های موجود روی outbound قبلی میمانند و IP خروجی عوض نمیشود.
- **ترافیک زنده:** از `StatusMessage` خود هسته (دقیق و per-direction) بهجای
  خواندن شمارندهٔ آداپتور که دسکتاپ مجبور است. وقتی `trafficAvailable=false`
  است اپ «هسته هنوز شمارشی نکرده» میگوید، نه «۰ بایت».
- **WireGuard/AmneziaWG:** `WgConf.kt` فایل `.conf` را به `endpoints[]` سینگ‌باکس
  ترجمه میکند (`type=wireguard` یا `type=awg`). **این یک واگرایی عمدی از دسکتاپ
  است:** دسکتاپ این خانواده را به wireproxy میدهد چون نسخهٔ قدیمی‌تر AWG در
  sing-box کار نمیکرد؛ روی اندروید پروسهٔ دومی وجود ندارد و هستهٔ ۱.۱۳ خودش
  `github.com/amnezia-vpn/amneziawg-go` را دارد (از باینری تأیید شد). قرارداد
  صداقت داور است: اگر handshake ترافیک نبرد، «وصل شد» گفته نمیشود.
- **اسپلیت تانلینگ per-app:** `include_package`/`exclude_package` که libbox به
  `VpnService.Builder` میدهد — **از دسکتاپ قابل‌اعتمادتر**، چون آنجا به تطبیق
  نام پروسه وابسته است و فقط وقتی کار میکند که یک اینترفیس مسیریافته بتواند
  جریان را به پروسه نسبت دهد.
- **پشتیبان‌گیری رمزشده:** همان فرمت دسکتاپ (`MVPNBAK1` + PBKDF2 210k +
  AES-256-GCM)، پس فایل بین ویندوز و موبایل جابه‌جا میشود. تأیید زنده: خروجی
  ۲.۱KB، هدر درست، UUID کانفیگ در فایل پیدا نمیشود، و بازگردانی روی اپ
  خالی‌شده ۳ کانفیگ + تنظیمات را برگرداند و همان کانفیگ بازگردانی‌شده وصل شد.
- **بقیه:** جستجو، مرتب‌سازی «سریع‌ترین» (لایه‌ای: گرم → تازه → کش → کهنه →
  ناشناخته → ناموفق)، تغییر نام، ویرایش لینک، اشتراک‌گذاری، بروزرسانی/حذف ساب،
  `AppLog` قابل دیدن در UI، تنظیمات DNS/اتصال خودکار/اتصال مجدد.

### سه شکست واقعی فاز ۳ (همه کامپایل میشدند، فقط روی دستگاه دیده شدند)

۱. **`fd.detachFd()`** → libbox مالک دیسکریپتور نمیشود، پس دیگر کسی نمیبستش:
   هر reconnect یک fd لیک میکرد و یک دستگاه tun جامانده میگذاشت
   (`tun0/tun1/tun2` بعد از سه اتصال). حالا `fd.fd` برمیگردد و `closeTun()`
   مالکیت را نگه میدارد.
۲. **حافظهٔ dedupe در `DefaultNetworkMonitor`** بین دو اتصال پاک نمیشد → هستهٔ
   تازه هیچ‌وقت اینترفیس پیش‌فرضش را نمیگرفت (monitor فکر میکرد eth0 قبلاً
   گزارش شده). علامتش گمراه‌کننده بود: اتصال اول سالم، هر reconnect تایم‌اوت.
۳. **۶۵۵۳۵ عددِ تأخیر نیست** → بیشینهٔ uint16 است و sing-box با آن شکست dial را
   اعلام میکند. بدون فیلتر، UI «۶۵۵۳۵ ms» نشان میداد؛ یعنی یک عدد ساختگی از
   همان جنسی که در ۳.۶.۹ ممنوع شد. `CoreClient.isRealDelay` قفلش کرد و کانفیگ
   ناموفق حالا هم «تایم‌اوت» میگیرد و هم عدد کش‌شدهاش پاک میشود.

**درس عمومی (به §۵):** یک UI-driver با مختصات ثابت خودش منبع گزارش دروغین است —
وقتی بنر اعلان ظاهر/ناپدید میشود همه‌چیز جابه‌جا میشود و نتیجه شبیه «قابلیت
اجرا نشد» دیده میشود. درایور تست باید عنصر را **با متن** پیدا کند و bounds را
همان لحظه بخواند.

### چیزی که عمداً روی اندروید پیاده نشد (با دلیل)

- **حالت‌های TUN/proxy-only/system-proxy:** `VpnService` خودش TUN است؛ نه API
  پروکسی سیستمی وجود دارد و نه میتوان پورت محلی را به اپ دیگری داد. یک سلکتور
  سه‌حالته اینجا فقط یک کلید بی‌اثر بود.
- **پورت پروکسی محلی، دیالوگ بستن پنجره، تری:** روی اندروید معنا ندارند.
- **IKEv2/OpenVPN:** خارج از توان sing-box؛ اپ صریحاً «پیاده نشده» میگوید.
- **SSH provision:** فاز ۴.

## ۹.۳ نسخه اندروید (۱۴ سپتامبر ۲۰۲۶ — دور دیباگ ساب/SSH، کامیت d2c66dd)

سه باگ واقعیِ گزارش کاربر («هرچی تست کردم وصل نشد») روی سرور واقعی
(3x-ui روی VPS) بازتولید، ریشه‌یابی و فیکس شد — همه با سنجهٔ زنده:

۱. **ساب base64 صفر کانفیگ ایمپورت میکرد.** `Subs.fetch` بدنه را دیکود
   نمیکرد و `importLinks` فقط خط‌های دارای `://` را میپارسد (دسکتاپ همین را
   در `fetchSubscriptionBody` حل کرده بود). فیکس: `Subs.decodeBody` با همان
   قرارداد + سختگیری اضافه (خروجی دیکود بدون `://` = برگشت به خام). بازتولید
   زنده: «هیچ لینکی پارس نشد» → بعد از فیکس: «۵ کانفیگ اضافه شد».
۲. **پنل 3x-ui کاربر در هر fetch ساب، پارامترهای Reality را عوض میکند**
   (sni/sid هر بار جدید — سه fetch، سه جفت متفاوت؛ اثبات: xray با پارامتر
   تازه → 204 در ۲.۷s، با پارامتر کهنه → timeout). یعنی لینک ایمپورتشده
   چند دقیقه بعد میمیرد. فیکس دو لایه: هویت پایدار merge
   (`protocol|address|port|secret` — sni/sid حذف) در `ingestLinks`، و
   **refresh خودکارِ بی‌صدای ساب قبل از اتصال** برای کانفیگ‌های اشتراکی
   (`refreshSubscriptionBlocking`). لاگ زنده: `0 added, 2 refreshed`.
۳. **اولین اتصال SSH همیشه «reject HostKey» میگرفت.** با
   `StrictHostKeyChecking=yes`، jsch روی میزبان ناشناس throw میکند بدون آنکه
   `add()` مخزن را صدا بزند — یعنی TOFU از روز اول مرده بود. فیکس: حالت
   `ask` + `TofuUserInfo` (میزبان ناشناخته → پین؛ mismatch → رد؛ `check`
   مخزن همچنان حکم نهایی CHANGED را میدهد). تست زنده: «SSH «MyVPS» OK» +
   پین SHA256 ثبت شد.

سنجههای اضافه: تستهای JVM جدید (بازتولید ساب واقعی، decodeBody، reroll-merge)
— ۱۲۵ تست سبز. **باقیمانده:** اتصالِ خود کانفیگ vless-Reality روی
sing-box/libbox اندروید هنوز ۲۰۴ نگرفته (با پارامتر تازه هم) در حالی که همان
لینک با xray روی دسکتاپ سبز است و کانفیگهای ws قبلاً روی همین امولاتور
connected شدهاند — مظنون بعدی: خروجی reality در sing-box 1.13 اندروید یا
MTU/stack. نکتهٔ تست UI: بنرهای اطلاعرسانی (مثل «سرور اضافه شد») چیدمان را
جابهجا میکنند؛ مختصات تپ باید همان لحظه از dump خوانده شود.
